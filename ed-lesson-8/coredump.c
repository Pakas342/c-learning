...
void helper(uintptr_t n, unsigned jump, char *numchar, unsigned org_jump,
            unsigned base);
unsigned inverse_atoi(uintptr_t n, char *numchar, unsigned base);
...
static void handler(int sig, siginfo_t *info, void *uctx) {
    ...
    if (info->si_code == SEGV_MAPERR) {
        char mess_2[] = "Error caused by no mapping at giving address\n";
        write(STDOUT_FILENO, mess_2, sizeof(mess_2));
    } else if (info->si_code == SEGV_ACCERR) {
        char mess_2[] =
            "Error caused by no permissions to read at giving address\n";
        write(STDOUT_FILENO, mess_2, sizeof(mess_2));
    } else {
    ...
    unsigned ptr_len = inverse_atoi((uintptr_t)info->si_addr, ptr + 2, 8);
    // hexes start with 0x;
    ptr[0] = '0';
    ptr[1] = 'x';
    ptr[ptr_len] = '\n';

    write(STDOUT_FILENO, ptr, ptr_len);
